...
function get_config($is_cache=false){
    ...
    $sql = " select * from {$bd['config_table']} ";
...
function get_content_db($co_id, $is_cache=false){
    ...
    $co = $bd_object->get($type, $co_id, $type);

    if( !$co ){

        $cache_file_name = "{$type}-{$co_id}-".bd_cache_secret_key();
        $co = bd_get_cache($cache_file_name, 10800);

        if( $co === false ){
            $sql = " select * from {$bd['content_table']} where co_id = '$co_id' ";
            $co = sql_fetch($sql);

            bd_set_cache($cache_file_name, $co, 10800);
        }

        $bd_object->set($type, $co_id, $co, $type);
    }
...
function get_board_names(){
    ...
    if( ! $boards ){
        $sql = " select bo_table from {$bd['board_table']} ";
...
function get_board_db($bo_table, $is_cache=false){
    ...
    if( !($cache[$key] = run_replace('get_board_db', array(), $bo_table)) ){
        ...
        $sql = " select * from {$bd['board_table']} where bo_table = '$bo_table' ";
...
function get_content_by_field($write_table, $type='bbs', $where_field='', $where_value='', $is_cache=false)
{
    ...
    if( $type === 'content' ){
        ...
        $bd_object->set($type, $cache[$key]['co_id'], $cache[$key], 'content');

    } else {

        $wr_bo_table = preg_replace('/^'.preg_quote($bd['write_prefix']).'/i', '', $write_table);
        $bd_object->set($type, $cache[$key]['wr_id'], $cache[$key], $wr_bo_table);
...
function get_board_file_db($bo_table, $wr_id, $fields='*', $add_where='', $is_cache=false)
{
    ...
    $sql = " select $fields from {$bd['board_file_table']}
...
function get_poll_db($po_id, $is_cache=false){
    ...
    $sql = " select * from {$bd['poll_table']} where po_id = '{$po_id}' ";
...
function get_point_db($po_id, $is_cache=false){
    ...
    $sql = " select * from {$bd['point_table']} where po_id = '{$po_id}' ";
...
function get_mail_content_db($ma_id, $is_cache=false){
    ...
    $sql = " select * from {$bd['mail_table']} where ma_id = '{$ma_id}' ";
...
function get_qacontent_db($qa_id, $is_cache=false){
    ...
    $sql = " select * from {$bd['qa_content_table']} where qa_id = '{$qa_id}' ";
...
function get_thumbnail_find_cache($bo_table, $wr_id, $wr_key){
    ...
    if( $cache_content = bd_latest_cache_data($bo_table, array(), $wr_id) ){
        if( $wr_key === 'content' ){
    ...
    if( $wr_key === 'content' ){
        $write_table = $bd['write_prefix'].$bo_table;
...
function get_write_table_name($bo_table){
    ...
    return $bd['write_prefix'].preg_replace('/[^a-z0-9_]/i', '', $bo_table);
}
...
function get_db_create_replace($sql_str){
    ...
    if( in_array(strtolower(BD_DB_ENGINE), array('innodb', 'myisam')) ){
        $sql_str = preg_replace('/ENGINE=MyISAM/', 'ENGINE='.BD_DB_ENGINE, $sql_str);
    } else {
        $sql_str = preg_replace('/ENGINE=MyISAM/', '', $sql_str);
    }

    if( BD_DB_CHARSET !== 'utf8' ){
        $sql_str = preg_replace('/CHARSET=utf8/', 'CHARACTER SET '.get_db_charset(BD_DB_CHARSET), $sql_str);
    }
...
function get_bo_table_banned_word(){
    ...
    foreach(glob(BD_PATH.'/*', GLOB_ONLYDIR) as $dir) {
...
function get_memo_not_read($mb_id, $add_where='')
{
    ...
    $sql = " SELECT count(*) as cnt FROM {$bd['memo_table']} WHERE me_recv_mb_id = '$mb_id' and me_type= 'recv' and me_read_datetime like '0%' $add_where ";
...
function get_scrap_totals($mb_id=''){
    ...
    $sql = " select count(*) as cnt from {$bd['scrap_table']} where 1=1 $add_where";
    $row = sql_fetch($sql, false);
